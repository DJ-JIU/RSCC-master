...
def _flash_attention_forward_or_none(
    **kwargs,
):
    """Fallback if flash_attn is not available - just pass through"""
    return None
...
def replace_qwen2_vl_attention_class():
    ...
    fa_fn = _flash_attention_forward if _FLASH_ATTN_AVAILABLE else _flash_attention_forward_or_none
...
def print_trainable_parameters(self) -> None:
    ...
    """
    # 兼容新旧版：新版中 Qwen2_5_VLModel 包含 language_model 子模块
    llm = getattr(self, 'language_model', self)

    # Check embed_tokens
    embed_tokens = getattr(llm, 'embed_tokens', None)
    ...
    # Get layers (might be in different locations)
    layers = getattr(llm, 'layers', getattr(self, 'layers', None))
    if layers is None:
        print("LLM Module - Unable to find decoder layers")
        return
