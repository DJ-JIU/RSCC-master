...
def set_model(model_args, model):
    ...
    if hasattr(model, 'visual'):
        ...
        # 新版: model.model.visual (transformers 4.51+)
        vision_model = model.model.visual
    else:
        raise AttributeError("Cannot find vision module in model")
    ...
    # merger 可能在 vision_model.merger 或 vision_model 其他子模块中
...
def train(attn_implementation="eager"):
    ...
    if "qwen2.5" or "RSCCM" in model_args.model_name_or_path.lower():
    ...
    if torch.distributed.get_rank() == 0:
        vision_model = model.visual if hasattr(model, 'visual') else model.model.visual
        vision_model.print_trainable_parameters()
        llm_model = model.model if hasattr(model.model, 'print_trainable_parameters') else model.model.language_model
