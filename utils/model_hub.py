...
try:
    ...
    Transformer = None; generate = None; MistralTokenizer = None
    UserMessage = None; TextChunk = None; ImageChunk = None; ChatCompletionRequest = None
...
def model_loader(
    ...
    if model_id == "moonshotai/Kimi-VL-A3B-Instruct":
        ...
        model_id.startswith("OpenGVLab/InternVL3-")
    ):
        # 优先从本地路径加载，否则从 huggingface hub 加载
        hf_kwargs = dict(
            torch_dtype=torch.bfloat16,
            use_flash_attn=True,
            trust_remote_code=True,
        )
        if "78B" in model_id or "38B" in model_id:
            hf_kwargs["device_map"] = device_map
        model = (
            AutoModel.from_pretrained(load_path, **hf_kwargs)
            .eval()
        )
        if "78B" not in model_id and "38B" not in model_id:
